...
{
    struct list_item* t;
    int t_data = list_entry(a, struct list_item, elem)->data;
    list_entry(a, struct list_item, elem)->data = list_entry(b, struct list_item, elem)->data;
    list_entry(b, struct list_item, elem)->data = t_data;
}

/* Shuffle elements. Using 2 iteration, using rnad() decide whether swap.*/
void
list_shuffle( struct list *list)
{ 
    struct list_elem *it1, *it2;
    srand((unsigned)time(NULL));
    for(it1 = list_begin(list); it1 != list_end(list); it1 = list_next(it1))
    {
